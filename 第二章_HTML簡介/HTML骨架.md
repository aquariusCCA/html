...
> 所屬章節：第二章｜HTML 簡介  
> 關鍵字：HTML骨架、骨架標籤、基本結構、`<!DOCTYPE html>`、`<html>`、`lang`、`<head>`、`<meta charset>`、`<title>`、`<body>`、HTML文檔、`.html`、`.htm`  
...
- 現代常見的基本結構會看到 `<!DOCTYPE html>`、`<html>`、`<head>`、`<meta charset>`、`<title>`、`<body>`。
...
- `lang`：通常寫在 `<html>` 上，用來標示頁面主要語言。
- `<head>`：放頁面的基本資訊。
- `<meta charset="UTF-8">`：宣告頁面使用的文字編碼，避免常見亂碼問題。
...
- `<html>` 包住整份頁面。
- `lang="zh-Hant"` 用來說明這份頁面的主要語言。
- `<head>` 放頁面的基本資訊。
- `<meta charset="UTF-8">` 用來宣告字元編碼，避免文字顯示異常。
- `<title>` 放頁面標題。
- `<body>` 放真正要顯示在頁面上的內容。
...
- 在本地建立與保存時，`HTML` 文檔最常見的副檔名是 `.html` 或 `.htm`。
- 瀏覽器的作用，是讀取 `HTML` 文檔並解析內容，再以網頁的形式顯示出來。
...
- 看到教材只寫 `title` 而沒寫 `meta charset` 或 `lang`，不代表那些資訊不重要；很多時候只是教材先用最簡化版本教骨架概念。
