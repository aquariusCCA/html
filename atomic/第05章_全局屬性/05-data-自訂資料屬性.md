...
> 來源：origin/第05章_全局屬性/05-HTML-data屬性.md / 全文
...
- 存儲的自定義數據能夠被頁面的 JavaScript 中利用，以創建更好的用戶體驗，不需要使用 Ajax 或去服務端查詢數據。
...
> ⚠️ 瀏覽器不會因為 `data-*` 屬性自動套用內建呈現或互動行為，但它會保留在 DOM 中，可由 JavaScript 的 `dataset` 或 `getAttribute()` 讀取。
