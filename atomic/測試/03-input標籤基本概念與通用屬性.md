...
- 多數可提交的表單控件會透過 `name` 搭配對應值提交資料；不同控件取得值的方式可能不同。
- `name` 是表單元素的名字。單選按鈕若要形成同一組互斥選項，需要使用相同的 name 值；複選框可依資料設計決定是否共用 name。
...
<form action="xxx.php" method="get">
  ...
  <!-- name 是表单元素的名字，同一组单选框需要有相同的 name 值 -->
  ...
  <!-- name 是表单元素的名字，复选框可依资料设计决定是否共用 name 值 -->
  <!-- checked 属性主要针对于单选框和复选框，主要作用是一打开页面，就可以默认选中某个表单元素 -->
  爱好:
  ...
  <!-- 重置按钮可以还原表单元素初始的默认状态 -->
  <!-- type 属性设置为 reset：重置按钮会将表单控件还原为初始默认状态 -->
  <!-- 属性 value 修改按钮显示的值 -->
