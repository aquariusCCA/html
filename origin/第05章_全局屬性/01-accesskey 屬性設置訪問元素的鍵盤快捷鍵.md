> 使用 accesskey 可以设置一个或者多个页面上的元素的快捷方式，其目的是让网页或者网站的熟客可以使用快捷键访问经常用到的元素。Windows 系统下快捷键的按法是：Alt+accesskey的属性值。
> - **⚠️ 注意：在不同操作系統中不同的瀏覽器中訪問快捷鍵的方式不同。**

- 在 HTML5 中 accesskey 屬性可用於任何 HTML 元素，它會驗證任何HTML元素，但不一定是有用。
