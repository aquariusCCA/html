...
1. 確認引用是否能唯一對應到目前章節 `assets/` 內的實體資產檔案。
2. 依照標準命名規則重新命名可確認的實體資產檔案。
3. 將 Markdown 或 HTML 中的引用路徑同步更新成章節內統一的資產路徑格式。
4. 回報已處理、無法匹配與需要人工確認的項目。

Markdown 或 HTML 中的原始引用路徑可能是舊格式，例如 `./images/a.png`、`./pdfs/a.pdf`、`./file.zip`。只要該引用可唯一對應到目前章節 `assets/` 內的實體檔案，就可以視為可處理資產，並統一改寫為 `./assets/<資產分類>/<標準檔名>`。
...
- `origin/` 依章節存放 `.md` 原始資料文件與該章節的 `assets/` 資產目錄。
- `origin/` 中的 `.md` 文件可以使用合法 Markdown 或 HTML 語法引用圖片、PDF、Excel、Word、音訊、影片、壓縮檔、程式碼檔案、外部連結或其他輔助資源。
- `origin/<章節>/` 的章節根目錄只放 `.md` 文件與 `assets/` 目錄，不會有 `origin/<章節>/notes/`。
...
3. 再確認引用是否能唯一對應到目前章節 `assets/` 內的實體檔案。
4. 再依照檔案內容計算 `hash6`。
5. 再檢查新檔名是否會與既有檔案衝突。
...
2. 如果原始引用路徑不是 `./assets/...`，只能在依原路徑正規化後可找到章節 `assets/` 內唯一檔案，或依原始檔名可在對應資產分類目錄中找到唯一檔案時處理。
...
5. 不要改寫空連結，例如 `href=""`。
6. 不要改寫明顯不是資產檔案的教學示例路徑，除非該路徑可依本文件匹配規則唯一對應到章節資產檔案。
...
| 程式碼或其他本地檔案 | 其他可確認存在於章節 `assets/` 內的檔案 | `./assets/files/` |
...
以下「改寫前」範例中的路徑代表 Markdown/HTML 內的舊引用格式；實體檔案仍須能唯一對應到目前章節 `assets/` 內的檔案，才可自動處理。
