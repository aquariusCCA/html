...
4. 是否沒有修改或移動 `assets/`。
5. 是否以主題概念作為主要切分粒度。
6. 是否把明顯放錯位置的內容移到語義正確的 atomic note。
7. 是否保留程式碼區塊、圖片、附件、表格與重要提示。
8. 是否將 atomic note 中的資產引用改為 `../../origin/<章節>/assets/...`。
9. 是否沒有改寫 fenced code block 內的示例路徑。
10. 是否為每篇 atomic note 保留來源資訊。
