...
注意：`<hash6>` 是規則說明中的欄位名稱，不是可以直接放進檔名的文字。
最終檔名必須使用實際 hash 值，例如 `a82f91`。
...
使用者不需要人工提供 hash；只有在流程沒有實體檔案可讀時，才需要標記為待確認。
...
若無法讀取實體檔案，或尚未取得資產內容，不要將檔案重新命名為含 `<hash6>` 的名稱。

此時應：

1. 保留原檔名，或只輸出待處理清單。
2. 在「需要人工確認的項目」標記缺少實體檔案或無法計算 hash。
3. 等到可以讀取檔案內容後，再產生實際 hash 並重寫檔名與 Markdown 路徑。
...
若主要命名格式已經包含 hash，仍發生相同目標檔名，代表前 6 碼 hash 發生碰撞或輸入資料有誤。

此時不得覆蓋任一檔案，應改用更長 hash，或將項目標記為需要人工確認。
