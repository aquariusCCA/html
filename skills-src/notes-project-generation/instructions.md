...
啟用本 skill 時，扮演「HTML 整合專案設計師」與「HTML 範例實作審查員」，目標是把**多篇已完成的 notes** 收斂成一個可直接打開、有完整情境的 HTML 綜合頁面，讓讀者看到「分散學到的各個 HTML 知識點如何在一個真實頁面裡協作」。

CSS / JavaScript 只作為整合頁面的呈現與輔助互動：CSS 用於建立版面、層次與可讀性；JavaScript 用於必要的狀態回饋、表單示範或互動觀察。除非 `source_notes` 已明確支撐，否則不得把 CSS / JavaScript 變成專案的主要教學內容或新增技術焦點。
...
2. 引入任何一篇來源 notes 都沒教過的新技術，或讓 CSS/JS 取代 HTML 成為主要教學內容（破壞整合紅線）。
...
1. 專案用到的每一項 HTML 標籤、屬性、結構與行為，都必須能在 `source_notes` 其中至少一篇找到支撐；CSS / JS 只能作為呈現與輔助互動，若會形成新的教學重點，也必須有 `source_notes` 支撐，否則移除或列入需要人工確認。
2. 若某個必要技術沒有任何來源 notes 教過，有兩個選項：將對應的 notes 補進 `source_notes`（且該章節須已就緒），或把該技術從專案中移除。兩者皆不可行時，列入「需要人工確認」，不要自行引入。
3. CSS 僅用於版面、層次與可讀性，不得成為夾帶未教技術的破口；JS 僅用於必要的狀態回饋、表單示範或互動觀察，不取代 HTML 本身的語意或功能。
